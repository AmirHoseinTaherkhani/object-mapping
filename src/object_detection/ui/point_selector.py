"""
Point Selection UI for Ground Truth Annotation
Interactive Streamlit interface for selecting reference points on video frames
"""
...
class PointSelector:
    """Handles point selection and coordinate annotation for homography calculation"""
    ...
    def load_video_frame(self, video_path: str, frame_number: int = 0) -> np.ndarray:
    ...
    def save_ground_truth_points(self, output_path: str, video_name: str, frame_number: int) -> None:
    ...
    def add_world_coordinate(self, x_coord: float, y_coord: float, description: str):
        """Add real-world coordinate for the last selected point"""
        if len(st.session_state.real_world_coords) < len(st.session_state.selected_points):
...
def main():
    st.set_page_config(
        page_title="Ground Truth Point Selector",
        page_icon="🎯",
        layout="wide"
    )
    
    st.title("🎯 Ground Truth Point Selector")
    ...
    with col2:
        ...
        if uploaded_video is not None:
            ...
            if st.button("📷 Load Frame"):
                frame_data = selector.load_video_frame(video_path, frame_number)
                if frame_data[0] is not None:
                    st.session_state.current_frame = frame_data[0]
                    st.rerun()
            
            # Point management
            st.header("📍 Point Management")
            ...
            if len(st.session_state.selected_points) > len(st.session_state.real_world_coords):
                st.header("🌍 Real-World Coordinates")
                ...
                with st.form(f"coord_form_{point_idx}"):
                    ...
                    if st.form_submit_button("💾 Add Coordinate"):
                        selector.add_world_coordinate(x_coord, y_coord, description)
            ...
            if len(st.session_state.selected_points) >= 4 and len(st.session_state.selected_points) == len(st.session_state.real_world_coords):
                ...
                if st.button("💾 Save Points"):
                    ...
                    selector.save_ground_truth_points(output_path, uploaded_video.name, frame_number)
    
    with col1:
        if st.session_state.current_frame is not None:
            st.header("🖱️ Click to Select Points")

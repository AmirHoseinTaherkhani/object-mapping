"""
Real-time vehicle & person counting — TensorRT backend (NVIDIA GPU).

Optimisations applied:
  1. TensorRT FP16 engine runs 5-10× faster than PyTorch on CUDA
...
Run export_tensorrt.py (or export_tensorrt.slurm on HPC) once first to
produce the .engine file ON THE SAME GPU MODEL as the demo machine.

Usage:
    python run_tensorrt.py                            # default video, skip-n=2
    python run_tensorrt.py --source 0                 # live webcam
    python run_tensorrt.py --source rtsp://...        # IP camera
    python run_tensorrt.py --skip-n 3 --no-display    # headless, skip 2/3 frames
    python run_tensorrt.py --weights path/to/model.engine --device 1
"""
...
ROOT     = HERE.parent
DEFAULT_WEIGHTS = ROOT / "models/weights/best_v3_merged.engine"
DEFAULT_VIDEO   = ROOT / "Demo/ANMR0006.mp4"
ROI_FILE        = ROOT / "counting_experiment/roi.json"
OUT_VID         = HERE / "output_tensorrt.mp4"
...
def parse_args():
    ...
    p.add_argument("--weights",     default=str(DEFAULT_WEIGHTS),
                   help="Path to .engine file (default: models/weights/best_v3_merged.engine)")
...
def main():
    ...
    if not weights.exists():
        sys.exit(
            f"Engine not found: {weights}\n"
            "Build it with:  python export_tensorrt.py\n"
            "Or on HPC:      sbatch export_tensorrt.slurm"
        )

    roi = load_roi(ROI_FILE)

    print(f"Loading TensorRT engine: {weights.name}  (device={args.device}) …")
    ...
    if not args.no_display:
        cv2.namedWindow("TensorRT — Real-time Counting", cv2.WINDOW_NORMAL)
    ...
    while cap.alive:
        ...
        if frame_idx % args.skip_n == 0:
            results  = model(frame, conf=conf_floor, device=args.device, verbose=False)[0]
        ...
        if not args.no_display:
            cv2.imshow("TensorRT — Real-time Counting", annotated)
    ...
    print(f"\n{'='*45}")
    print("FINAL COUNTS  (TensorRT + ByteTrack)")
    print(f"{'='*45}")
